...
using System.Web;
using Flurl.Http;
using Infrastructure.Common.Core;
...
namespace Infrastructure.Middleware.Core
{
    public class ApiSubActionFilter : IAsyncActionFilter
    {
        ...
        {
            _url = configurationRoot["BasicSetting:Fabio"];
        }
            ...
            if (context.HttpContext.Request.Path.HasValue)
            {
                ...
                if (path.IndexOf("/open.", StringComparison.Ordinal) > -1)
                {
                    ...
                    if (string.IsNullOrEmpty(appKey) || string.IsNullOrEmpty(appSecret) ||
                        string.IsNullOrEmpty(timeStamp) || string.IsNullOrEmpty(sign))
                    {
                    ...
                    var result =
                        await
                            $"{_url}/school_precaution_org/inside.org.merchant.valid?appKey={HttpUtility.UrlEncode(appKey)}&appSecret={HttpUtility.UrlEncode(appSecret)}&preSign={HttpUtility.UrlEncode(preSign)}&sign={HttpUtility.UrlEncode(sign)}"
                                .GetJsonAsync<SubmitResult>();
                    if (result.IsSuccess)
                    {
